...
export default class PayInfo extends React.Component<PayInfoProps, any> {
  ...
  componentWillReceiveProps(nextProps) {
    if (
      nextProps.goodsId !== this.props.goodsId ||
      nextProps.goodsType !== this.props.goodsType
    ) {
        ...
        if (res.code === 200) {
          this.setState(res.msg, () => {
            ...
            if (!_.isEmpty(res.msg.autoCoupons) && res.msg.coupons) {
              this.handleAutoChooseCoupon(
                res.msg.autoCoupons,
                res.msg.multiCoupons,
                goodsType,
                goodsId
              )
            }
  ...
  handleClickChooseAll() {
    ...
    if (multiCoupons && !_.isEmpty(coupons)) {
      ...
      let param = {
        goodsId: goodsId,
        goodsType: goodsType,
        couponsIdGroup: chose.couponsIdGroup,
        priceActivityId: priceActivityId,
      }
      dispatch(startLoad())
      calculateCoupons(param)
  ...
  render() {
    ...
    const renderPrice = (fee, final, free, initPrice) => {
      ...
      if (initPrice && !_.isEqual(initPrice, fee)) {
        priceArr.push(
          <span className="discard" key={0}>{`原价：¥${numeral(
            initPrice
          ).format('0.00')}元`}</span>
        )
      } else if ((final || free) && !_.isEqual(final, fee)) {
        priceArr.push(
          <span className="discard" key={0}>{`¥ ${numeral(fee).format(
            '0.00'
          )}元`}</span>
        )
      }
      ...
      if (priceArr.length >= 1) {
        priceArr.push(
          ...
          >{`¥ ${numeral(
            (final || free) && !_.isEqual(final, fee) ? final : fee
          ).format('0.00')}元`}</span>
        )
      } else {
        priceArr.push(
          <span className="final" key={0}>{`¥ ${numeral(fee).format(
            '0.00'
          )}元`}</span>
        )
      }
      if (this.props.priceTips) {
        priceArr.push(
          <div key={priceArr.length} className="price-tips">
            {this.props.priceTips}
          </div>
        )
      }
      return priceArr
    ...
    if (
      window.ENV.osName === 'android' &&
      parseFloat(window.ENV.osVersion) <= 4.3
    ) {
      // <!-- 安卓4.3 以下 -->
      return (
        <div className={`simple-pay-info ${show ? 'show' : ''}`}>
          ...
          <div className="main-container">
            ...
            <div className="content">
              <div
                className={`price item ${
                  this.props.priceTips ? 'show-tips' : ''
                }`}
              >
              ...
              <div className={`coupon item`}>
                {coupons && chose && chose.used
                  ? `优惠券：¥${numeral(chose.total).format('0.00')}元`
                  : '选择优惠券'}
              </div>
            </div>
            <ul className={`coupon-list`}>
              {coupons
                ? coupons.map((item, seq) => {
                    return (
                      <li className="coupon" key={seq}>
                        ...
                        <div
                          className={`btn ${couponChosen(item) ? 'chose' : ''}`}
            ...
            <span
              style={{paddingLeft: '15px', fontSize: '15px', color: '#333'}}
            >
              支付方式:
            </span>
            <br />
            <ul className="pay-type-list">
              <li
                className={classnames({choose: payType == PayType.WECHAT})}
                onClick={() => this.setState({payType: PayType.WECHAT})}
              >
                微信
              </li>
              {fee > MULTI_PAY_TYPE_PRICE && (
                <li
                  className={classnames({choose: payType == PayType.ALIPAY})}
                  onClick={() => this.setState({payType: PayType.ALIPAY})}
                >
                  支付宝/*<span className="pay-type-tips">(支持花呗分期)</span>
                  */
                </li>
              )}
              {fee >= KFQ_PAY_PRICE_RANGE[0] &&
                fee <= KFQ_PAY_PRICE_RANGE[1] &&
                showKfq && (
                  <li
                    className={classnames({choose: payType == PayType.KFQ})}
                    onClick={() => this.setState({payType: PayType.KFQ})}
                  >
                    银联分期
                  </li>
                )}
              {showHuabei && (
                <li
                  className={classnames({choose: payType == PayType.HUABEI_3})}
                  onClick={() => this.setState({payType: PayType.HUABEI_3})}
                >
                  花呗分期(3期)
                </li>
              )}
              {showHuabei && (
                <li
                  className={classnames({choose: payType == PayType.HUABEI_6})}
                  onClick={() => this.setState({payType: PayType.HUABEI_6})}
                >
                  花呗分期(6期)
                </li>
              )}
              {showHuabei && (
                <li
                  className={classnames({choose: payType == PayType.HUABEI_12})}
                  onClick={() => this.setState({payType: PayType.HUABEI_12})}
                >
                  花呗分期(12期)
                </li>
              )}
          ...
          >
            <div
              className={classnames('header', {
                openCoupon: openCoupon || openPayType,
            ...
            >
              ...
              <div
                className={classnames('price', 'item', {
                  'show-tips': this.props.priceTips,
                })}
              >
              ...
              {hasCoupons && (
                <div
                  className={classnames('has-arrow', 'coupon', 'item', {
                    open: openCoupon,
                    hidden: openPayType,
                    'just-open-pay-type': justOpenPayType,
                  })}
                  onClick={() =>
                    this.setState({openCoupon: !this.state.openCoupon})
                  }
                >
                  {chose && chose.used
                    ? `优惠券：¥${numeral(chose.total).format('0.00')}元`
                  ...
                  {openCoupon && multiCoupons && (
                    <div className="coupon-manual-close">确认</div>
                  )}
                </div>
              )}
              ...
              {multiCoupons && (
                <div className="choose-all-wrapper">
                  <div className="choose-area">
                    ...
                    <div
                      className={classnames('choose-all', {chose: chooseAll})}
                ...
                coupons.map((item, seq) => {
                  return (
                    <li className="coupon" key={seq}>
                      ...
                      <div
                        className={classnames('chose-btn', {
                          'multi-chooses': multiCoupons,
                          chose: couponChosen(item),
                        })}
                        onClick={() => this.handleClickChooseCoupon(item)}
                      >
                        <div
            ...
            >
              ...
              <li className="pay-type-item">
                <div className="pay-type-info">
                  ...
                  <div className="pay-type-name">
                    支付宝
                    {/*<span className="pay-type-tips">(支持花呗分期)</span>*/}
                  </div>
                </div>
                <div
                  className={classnames('chose-btn', {
