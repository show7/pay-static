...
export default class PayGift extends React.Component<any, any> {
    ...
    constructor(props) {
        ...
        this.state = {
            showPayFlay: false,
            choseFlay: false,
            wayList: [{id: PayType.WECHAT, name: '微信'}, {id: PayType.ALIPAY, name: '支付宝'}],
    ...
    handleClickPay(){
        ...
        loadPaymentParam(param).then((res)=>{
            if (res.code===200){
                ...
                chosedWayId === PayType.WECHAT ?  this.weChatPay(signParams,goodsType): window.location.href = `/pay/alipay/rise?orderId=${productId}&goto=${encodeURIComponent(signParams.alipayUrl)}`;
            }
        });
    }
     ...
     */
    handlePayDone(){
      window.location.href=`https://${window.location.hostname}/rise/activity/static/promotion/shareposter?memberTypeId=17`;
    }
    /**
     * 支付关闭回调
     */
    ...
    render(){
       const {showPayFlay,choseFlay,wayList,chosedWayId,totalNum,unitPrice,projectName,showShare,sellImgs} =this.state;
        return(
            <div className="pay-gift">
                      ...
                      <p className="name">{projectName}</p>
                        <div className="unit-content">
                            ...
                            <div className="pay-way">
                                <span>支付方式</span>
                                <div className="chose-way" onClick={()=>{this.setState({choseFlay:true})}}>
                                    {chosedWayId == PayType.WECHAT ? <img src="https://static.iqycamp.com/pay_type_icon_wechat-tk3zx9gt.png" alt="微信"/> :
                                        <img src="https://static.iqycamp.com/pay_type_icon_ali-yvkeubk4.png" alt="支付宝"/>}
                                    <span>{chosedWayId == PayType.WECHAT ? '微信支付':'支付宝支付'}</span>
                                    <img className="down" src="https://static.iqycamp.com/down-s0t7kgwa.png" alt="选择"/>
                                </div>
                            </div>
                            <div className="button-pay">
                                <p onClick={()=>{this.handleClickPay()}}>确认支付</p>
                            </div>
                        </div>
                    </div>
                </div>
                ...
                <div className={`chose-payway ${choseFlay ? 'active':''}`}>
                    <ul>
                        { wayList.map((item,index)=>{
                            return (
                                <li className={`${chosedWayId == item.id ? 'active':''}`} key={item.id} onClick={()=>{this.handleGetWay(item.id)}}>{item.name}</li>
                            )
                        })}
                    </ul>
                </div>
                ...
                {
                    showShare &&
                    <div className="share-mask" onClick={()=>this.setState({showShare:false})}>
                        <div className="share-mask-box">
                            <p>你已经成功购买礼品卡，点击右上角，转发给好友进行赠送吧！
                                未赠送的礼品卡将会放在“个人中心”-“我的赠卡”</p>
                        </div>
                    </div>
                }
