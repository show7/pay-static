...
export default class PayL3 extends Component<any, any> {
  constructor() {
    ...
    this.state = {
      ...
      subscribeAlertTips: {
        ...
        qrCode:
          'https://static.iqycamp.com/images/qrcode_qwzswyh.jpeg?imageslim',
      },
      ...
      if (res.code == 200) {
        this.setState({task: res.msg}, () => {
          configShare(
            ...
            `https://${window.location.hostname}/pay/thought?riseId=${
              window.ENV.riseId
            }&type=2`,
  ...
  handleClickOpenPayInfo(showId) {
    ...
    // 先检查是否能够支付
    checkRiseMember(showId, riseId, type)
      .then(res => {
        dispatch(endLoad())
        if (res.code === 200) {
  ...
  render() {
    ...
    return (
      <div className="plus-pay">
        {/* <div
          className="receive-benefits-btn"
          onClick={() => this.setState({closeQrCode: true})}
        >
          <div className="background-cirle">
            <img
              src="https://static.iqycamp.com/礼物icon@2x-n3kzc9h8.png"
              alt=""
            />
          </div>
          <div>
        ...
        {subscribe && (
          <SubscribeAlert
            tips={subscribeAlertTips.tips}
            qrCode={subscribeAlertTips.qrCode}
        ...
        {timeOut && (
          <div
            ...
            style={{
              background:
                'url("https://static.iqycamp.com/images/riseMemberTimeOut.png?imageslim") center' +
                ' center/100% 100%',
            }}
          />
        )}
        {showErr && (
          <div className="mask" onClick={() => this.setState({showErr: false})}>
            <div className="tips">
              ...
              <br />{' '}
              2如果遇到“支付问题”，扫码联系小黑，并将出现问题的截图发给小黑
              <br />
            </div>
            <img
              className="xiaoQ"
